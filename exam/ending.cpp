...
string Ending::judgeEnding(const Player& player, const ExamResult& result)
{
    // result是期末考试ExamResult
    const int score = result.getscore();
    int health = player.getStats().health; //读取玩家健康状态，可以作为结局条件
    if (health <= 0)
    {
...
void Ending::showEnding(const string& endingId)
{
    cout << "\n========== 游戏结局 ==========\n";
